/*
 * GSM 07.10 Implementation with User Space Serial Port
 *
 * Code heavily based on gsmMuxd written by
 * Copyright (C) 2010 Edward.Xue <xuedongyi1983@gmail.com>
 *
 ...
 * Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, CN.
 */
...
//msc
char msc_c_1[4] = {0xE3,0x05,0x07,0x0D};
char msc_c_2[4] = {0xE3,0x05,0x0B,0x0D};

#define LOGMUX(lvl,f,...) do{if(/*lvl == LOG_EMERG*/0){\
								if (logtofile){\
...
#define PTS_NAME_LINK   1   //增加pts name link 的支持 @dxj
...
// type of frames
#define GSM0710_TYPE_SABM 0x3F//47 Set Asynchronous Balanced Mode
...
static int syslog_level = LOG_EMERG;
static FILE * muxlogfile;
static int vir_ports = 5; /* number of virtual ports to create */
//#if PTS_NAME_LINK
static char* ptsSymlinkPre = 0;//逻辑通道自定义的符号链接名称前缀 @dxj
//#endif
/*misc global vars */
...
static int syslogdump(
	...
	unsigned int length)
{
if (/*0>syslog_level*/0) /*No need for all frame logging if it's not to be seen */
	return 0;
...
/* flag, GSM0710_EA=1 C channel, frame type, length 1-2 */
	...
	int c;
       ...
       LOGMUX(LOG_INFO, "prefix[1]  = 0x%x",prefix[1]);
...
/* length */
		...
		if (c != prefix_length)
		{
			LOGMUX(LOG_WARNING, "Couldn't write the whole prefix to the serial port for the virtual port %d. Wrote only %d bytes",channel, c);
			return 0;
		}
		if (length > 0)
		{
		syslogdump(">s ", input,length); /* syslogdump for basic mode */
			...
			if (length != c)
			{
				LOGMUX(LOG_WARNING, "Couldn't write all data to the serial port from the virtual port %d. Wrote only %d bytes",channel, c);
				return 0;
			}
		}
		...
		if (c != 2)
		{
			LOGMUX(LOG_WARNING, "Couldn't write the whole postfix to the serial port for the virtual port %d. Wrote only %d bytes",channel, c);
			return 0;
		}
#else
/* Modified acording PATCH CRC checksum */
...
/* length */
		...
		//syslogdump(">s ", input,length); /* syslogdump for basic mode */
			//c = write(serial.fd, input, length);
			memcpy(buf+prefix_length,input,length);
			memcpy(buf+prefix_length+length,postfix,2);

		syslogdump(">s ", buf,(prefix_length+length+2)); /* syslogdump for basic mode */
		c = write(serial.fd, buf, (prefix_length+length+2));
		if (c != (prefix_length+length+2))
		{
			LOGMUX(LOG_WARNING, "Couldn't write the whole postfix to the serial port for the virtual port %d. Wrote only %d bytes",channel, c);
			return 0;
		}
	...
	else/* cmux_mode */
	{
		...
		if (c != offs)
		{
			LOGMUX(LOG_WARNING, "Couldn't write the whole advanced packet to the serial port for the virtual port %d. Wrote only %d bytes",channel, c);
			return 0;
		}
	}
	...
	unsigned char type)
{
	...
	LOGMUX(LOG_DEBUG, "Enter");
/* flag, GSM0710_EA=1 C channel, frame type, length 1-2 */
	unsigned char prefix[5] = { GSM0710_FRAME_FLAG, GSM0710_EA , 0, 0, 0 };
	unsigned char postfix[2] = { 0xFF, GSM0710_FRAME_FLAG };
	int prefix_length = 4;
	int c;
...
/* try to write 5 times */
	...
	{
	       //LOGMUX(LOG_INFO,"call write_frame()     .1 ");
...
static int logical_channel_close(Channel* channel)
{
	...
	//#if PTS_NAME_LINK
	if (channel->ptsLinkName != NULL)
		free(channel->ptsLinkName);
	channel->ptsLinkName = NULL;
	//#endif
	if (channel->tmp != NULL)
		free(channel->tmp);
	channel->tmp = NULL;
...
/*
    ...
    @dxj
*/
char * create_SymlinkName(int idx, char* ptsName) 
{

    static char symLinkName[100];
    
    if (ptsSymlinkPre == NULL) 
    {
        //sprintf(symLinkName, "%s%d", "ptys", idx);
        //return symLinkName;
        return NULL;
    }
    
    sprintf(symLinkName, "%s%d", ptsSymlinkPre, idx);

    unlink(symLinkName);
	if (symlink(ptsName, symLinkName) != 0)
	{
	    LOGMUX(LOG_WARNING, "symlink failed %s",symLinkName);
	}
	else
	{
	    LOGMUX(LOG_WARNING, "symlink success %s",symLinkName);
	}
    return symLinkName;
}
//#endif
...
int restart_pty_interface(Channel* channel)
{
	if (channel->fd < 0) // is this channel free?
				...
				//#if PTS_NAME_LINK
					channel->ptsLinkName = strdup(create_SymlinkName(channel->id, channel->ptsname));
				//#endif
...
{
	LOGMUX(LOG_DEBUG, "Enter");
	LOGMUX(LOG_DEBUG, "pseudo_device_read() Enter ~~~~~~~~~~\n");
	Channel* channel = (Channel*)vargp;
		...
		{   
			...
			packet_len = len;
		       do{
			      i++; 	
			}while(*(buf + channel->remaining + i) != 0x7E);
			if(i < (len-1)){
				packes = 2;
				flag = 1;
			}else{
			...
			for(;packes>0;packes --){
				 ...
				 packet1_len = packet_len - packet2_len;
				 len = packet1_len;
				 offset = packet2_len;
			     }
			  if(flag == 0)
			  	offset = 0;
			  //LOGMUX(LOG_ALERT, "--------->buf 0x%x",buf);
		         LOGMUX(LOG_ALERT, "--------->packes %d",packes);		 
			  LOGMUX(LOG_ALERT, "--------->packet_len %d",packet_len);
			  LOGMUX(LOG_ALERT, "--------->packet2_len %d",packet2_len);
			  LOGMUX(LOG_ALERT, "--------->packet1_len %d",packet1_len);
			  LOGMUX(LOG_ALERT, "--------->channel->remaining %d",channel->remaining);
			...
			if (len + channel->remaining > 0)
				channel->remaining = handle_channel_data(buf + offset, len + channel->remaining, channel->id);
...
#if 0
static int open_pty(int i, pthread_t * thread_id)
{
	...
	if (serial.state == MUX_STATE_MUXING)
		for (i=1;i<GSM0710_MAX_CHANNELS;i++)
			if (channellist[i].fd < 0) // is this channel free?
			{
				...
				LOGMUX(LOG_INFO,"c_alloc_channel channellist[%d].devicename = %s\r\n",i,channellist[i].devicename);
				LOGMUX(LOG_INFO,"c_alloc_channel pts = %s\r\n",pts);
				//LOGMUX(LOG_INFO,"c_alloc_channel channellist[i].ptsname = %s\r\n",i,channellist[i].ptsname);
				
                            //#if PTS_NAME_LINK
                            channellist[i].ptsLinkName = strdup(create_SymlinkName(i, channellist[i].ptsname));
                            //#endif
                            
                            //channellist[i].id = i; //增加逻辑通道号的记录? @dxj
#if 1
				struct termios options;
				tcgetattr(channellist[i].fd, &options); //get the parameters
				options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); //set raw input
				...
				tcsetattr(channellist[i].fd, TCSANOW, &options);
                            ...
                            struct termios options;
              		// get the parameters
              		tcgetattr(channellist[i].fd, &options);
              		// set raw input
                ...
                poll_thread_arg->read_function_arg = (void *) (channellist+i);
              	...
              	LOGMUX(LOG_DEBUG, "Thread is running and listening on %s", channellist[i].ptsname);
                            //LOGMUX(LOG_INFO,"call write_frame()     .3 ");
				write_frame(i, NULL, 0, GSM0710_TYPE_SABM | GSM0710_PF); //should be moved?? messy
				LOGMUX(LOG_INFO, "Connecting %s to virtual channel %d for %s on %s",
					channellist[i].ptsname, channellist[i].id, channellist[i].origin, serial.devicename);
				// msc
				LOGMUX(LOG_INFO, "send msc frame i = %d",i);
				if(i == 1)
				{
				 write_frame(0, msc_c_1, 4, GSM0710_TYPE_UIH); //should be moved?? messy
				}
				else if(i ==  2)
				{
				 write_frame(0, msc_c_2, 4, GSM0710_TYPE_UIH); //should be moved?? messy
				}
				//end of add
                            asprintf(&cmd, "chmod 777 %s;", channellist[i].ptsname);
                            system(cmd);
                            free(cmd);
	...
	GSM0710_Buffer * buf)
{
	...
	LOGMUX(LOG_DEBUG, "Enter");
	int i;
	LOGMUX(LOG_ALERT, "^^^^^^^^^^^^^^^^^^^^^^ Enter ^^^^^^^^^^^^^^^^^^^^^^");
	LOGMUX(LOG_ALERT, "<------> buf->readp =  %d ,buf->datacount = %d",(buf->readp - buf->data),buf->datacount);
	//for(i = 0;i<5;i++)
	//LOGMUX(LOG_ALERT, "*%d.  buf->readp =  0x%x",(buf->readp+i),*(buf->readp+i));
	//LOGMUX(LOG_DEBUG, "&&&&& buf->readp =  0x%x ,buf->datacount = 0x%x",buf->readp,buf->datacount);
/*Find start flag*/
	...
	{
		LOGMUX(LOG_ALERT, "!!!! Leave. No start frame 0xf9 found in bytes stored in GSM0710 buffer");
		//add 
...
/* Okay, we're ready to analyze a proper frame header */
	...
	if (local_datacount >= length_needed) /* enough data stored for 0710 frame header+footer? */
	{
		if ((frame = (GSM0710_Frame*)malloc(sizeof(GSM0710_Frame))) != NULL)
		{
			...
			{
				LOGMUX(LOG_ALERT, "!!!! Dropping frame: Corrupt! Channel Addr. field indicated %d, which does not exist",frame->channel);
				//destroy_frame(frame);  //mod debug6
				free(frame);
				buf->flag_found = 0;
				buf->dropped_count++;
			...
			LOGMUX(LOG_ALERT, "###### frame->channel = 0x%x ######",frame->channel);
			LOGMUX(LOG_ALERT, "###### frame->control = 0x%x ######",frame->control);
			LOGMUX(LOG_ALERT, "### 1byte ### frame->length = 0x%x ######",frame->length);
			
		}
		else
			LOGMUX(LOG_ALERT, "!!!! Out of memory, when allocating space for frame");
		if ((*local_readp & 1) == 0)/*if frame payload length byte extension bit not set, a 2nd length byte is in header*/
		{
			...
			fcs = r_crctable[fcs^*local_readp];
			LOGMUX(LOG_ALERT, "### 2byte ### frame->length = 0x%x ######",frame->length);
		}
		length_needed += frame->length; /*length_needed : 1 length byte + payload + 1 fcs byte + 1 end frame flag */
		LOGMUX(LOG_ALERT, "length_needed: %d, available in local_datacount: %d",length_needed,local_datacount);

		if (frame->length > cmux_N1) /* Field Sanity check if payload is bigger than the max size negotiated in +CMUX */
		{
			LOGMUX(LOG_ALERT, "!!!! Dropping frame: Corrupt! Length field indicated %d. Max %d allowed",frame->length, cmux_N1);
			//destroy_frame(frame);
			free(frame);
			buf->flag_found = 0;
			buf->dropped_count++;
		...
		LOGMUX(LOG_ALERT, "local_datacount = %d,length_needed = %d",local_datacount,length_needed);
		if (!(local_datacount >= length_needed))
		{
			free(frame);
			LOGMUX(LOG_ALERT, "!!!! Leave, frame extraction cancelled. Frame not completely stored in re-assembly buffer yet");
                     	LOGMUX(LOG_ALERT, "<---      buf->readp =  %d ,buf->datacount = %d",(buf->readp - buf->data),buf->datacount);
                     	LOGMUX(LOG_ALERT, "<---^^^^^^^^^^^^^^^^^^ Leave, frame have not  ok^^^^^^^^^^^^^^^^^^^^^^");
			//add
			pthread_mutex_lock(&bufferready_lock);
			if (buf->input_sleeping == 1)
			{
...
/*Okay, done with the frame header. Start extracting the payload data */
		if (frame->length > 0)
		{
			...
			{
				LOGMUX(LOG_ALERT, "!!!! Out of memory, when allocating space for frame data");
				frame->length = 0;
			}
		}
/*Okay, check FCS*/
		if (r_crctable[fcs ^ (*local_readp)] != 0xCF)
		{
			...
			{
				LOGMUX(LOG_ALERT, "!!!! Dropping frame: Corrupt! End flag not present and FCS mismatch.");
				destroy_frame(frame);
				buf->flag_found = 0;
				buf->dropped_count++;
				goto update_buffer_dropping_frame; /* throw whole frame away, up until and incl. local_readp */
			...
			{
			LOGMUX(LOG_ALERT, "!!!! Dropping frame: FCS doesn't match");
			destroy_frame(frame);
			buf->flag_found = 0;
			buf->dropped_count++;
			goto update_buffer_dropping_frame; /* throw whole frame away, up until and incl. local_readp */
...
/*Okay, check end flag */
			...
			{
				LOGMUX(LOG_ALERT, "!!!! Dropping frame: End flag not present. Instead: %d", *local_readp);
				destroy_frame(frame);
				buf->flag_found = 0;
				buf->dropped_count++;
	...
	{
		LOGMUX(LOG_ALERT, "!!!! Leave, not enough bytes stored in buffer for header information yet");
		return NULL;
	}
...
/* Everything went fine, update GSM0710 buffer pointer and counter */
	...
	LOGMUX(LOG_DEBUG, "Leave, frame found");
       ...
       LOGMUX(LOG_ALERT, "<------> buf->readp =  %d ,buf->datacount = %d",(buf->readp - buf->data),buf->datacount);
	LOGMUX(LOG_ALERT, "^^^^^^^^^^^^^^^^^^^^^^ Leave, frame found ^^^^^^^^^^^^^^^^^^^^^^");
	return frame;


	update_buffer_dropping_frame:
		/*Update GSM0710 buffer pointer and counter */
		LOGMUX(LOG_ERR, "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$  update_buffer_dropping_frame $$$$$$$$$$$$$$$$$$$$$$$$$$$$$4");
		pthread_mutex_lock(&datacount_lock); 
		buf->readp = local_readp;
		buf->datacount -= (local_datacount_backup - local_datacount); /* subtract whatever we analyzed */
		pthread_mutex_unlock(&datacount_lock); 
...
#else
SYSCHECK(tcdrain(serial_device_fd));//tcdrain函数等待所有输出都被发送
#endif
...
//op.arg |= USSP_DCD;
			...
			default:
				...
				if ((response = malloc(sizeof(char) * (2 + type_length))) != NULL)
				{
					...
					//LOGMUX(LOG_INFO,"call write_frame()     .4 ");
...
//acknowledge the command
				frame->data[0] = frame->data[0] & ~GSM0710_CR;
                            ...
                            LOGMUX(LOG_INFO,"call write_frame()     .5 ");
...
* Return:   number of frames extracted
  mux协议的命令解析处理,消费in_buf数据
  从in_buf里提取frame数据，并进行分析:假如是逻辑通道上传输的数据，则发送给相应的逻辑通道，假如是控制通道，则进行相应的处理(包括应答)；
*/
int extract_frames(GSM0710_Buffer* buf)
{
		...
		: gsm0710_base_buffer_get_frame(buf)))
	{
		...
		if (buf->input_sleeping == 1)
		{
		    //in_buf满的时候进入sleep，这时候进行唤醒.将newdataready_signal改为bufferready_signal @dxj
		    pthread_cond_signal(&bufferready_signal);
			//pthread_cond_signal(&newdataready_signal);  
		}
		pthread_mutex_unlock(&bufferready_lock);
		...
		if ((GSM0710_FRAME_IS(GSM0710_TYPE_UI, frame) || GSM0710_FRAME_IS(GSM0710_TYPE_UIH, frame)))
		{
			...
			{
			       LOGMUX(LOG_ALERT, "***********************************************************");
				LOGMUX(LOG_ALERT, "Writing %d byte frame received on channel %d to %s",frame->length,frame->channel, channellist[frame->channel].ptsname);

                            syslogdump("<s ", (unsigned char *)frame->data, frame->length);
                            LOGMUX(LOG_ALERT, "***********************************************************");
				...
				LOGMUX(LOG_DEBUG, "Writing %d byte frame received on channel %d to %s",frame->length,frame->channel, channellist[frame->channel].ptsname);
			       ...
			       if ((write_result = write(channellist[frame->channel].fd, frame->data, frame->length)) >= 0)//接收到数据，上传逻辑通道frame->channel
				{
...
//close channels
			...
			case GSM0710_TYPE_DISC:
				if (channellist[frame->channel].opened)
				{
					...
					//LOGMUX(LOG_INFO,"call write_frame()     .7 ");
...
//channel already closed
					LOGMUX(LOG_WARNING, "Received DISC even though channel %d for %s was already closed",
							frame->channel, channellist[frame->channel].origin);
                                   //LOGMUX(LOG_INFO,"call write_frame()     .8 ");
...
//channel already opened
					LOGMUX(LOG_WARNING, "Received SABM even though channel %d for %s was already closed",
						frame->channel, channellist[frame->channel].origin);
				channellist[frame->channel].opened = 1;
				//LOGMUX(LOG_INFO,"call write_frame()     .9 ");
				write_frame(frame->channel, NULL, 0, GSM0710_TYPE_UA | GSM0710_PF);
				break;
			}
		}
...
void* assemble_frame_thread(void * vargp)
{
	...
	while(buf!=NULL)
	{
		...
		{
			LOGMUX(LOG_ALERT,"assemble_frame_thread put to sleep. GSM0710 buffer stored %d, newdataready: %d", buf->datacount, buf->newdataready);
			pthread_cond_wait(&newdataready_signal,&newdataready_lock); /* sleep until signalled by thread_serial_device_read() */
			LOGMUX(LOG_ALERT,"assemble_frame_thread awoken. GSM0710 buffer stored %d, newdataready: %d", buf->datacount, buf->newdataready);
		}
	...
	int param)
{
	...
	{
	case SIGPIPE:
		LOGMUX(LOG_DEBUG,"signal_treatment  SIGPIPE\r\n");
		exit(0);
	break;
	case SIGHUP:
		LOGMUX(LOG_DEBUG,"signal_treatment  SIGHUP\r\n");
//reread the configuration files
	...
	case SIGUSR1:
		//exit(0);
//sig_term(param);
             LOGMUX(LOG_DEBUG,"signal_treatment  SIGINT SIGTERM SIGUSR1\r\n");
             set_main_exit_signal(1);
	break;
	case SIGKILL:
		LOGMUX(LOG_DEBUG,"signal_treatment  SIGKILL\r\n");
	default:
		exit(0);
	break;
	}
	...
	LOGMUX(LOG_DEBUG, "Enter");
	{
		switch (serial->state)
		{
			case MUX_STATE_MUXING:
			{
				...
				int length;
				LOGMUX(LOG_DEBUG, "~~~~  Serial Data allow read max data size = %d ~~~~",min(length, sizeof(buf)));
				...
				{   
				    //读取 HIF或Dual Ram数据，没数据时应该阻塞。是否用poll? @dxj
					if((len = read(serial->fd, buf, min(length, sizeof(buf)))) > 0)
					{
					       LOGMUX(LOG_DEBUG, "Serial Data read data success ~~~~ length ~~~~ = %d",len);
						syslogdump("<s ", buf, len);
						gsm0710_buffer_write(serial->in_buf, buf, len);//写到缓冲serial->in_buf，并通知解析进程assemble_frame_thread 进行消费处理
					}
				...
				else    //等待extract_frames 里消费数据后，满足条件bufferready_signal
				{
...
#else
int open_Hif_device(Serial* serial)
{
	LOGMUX(LOG_DEBUG, "Enter");
	unsigned int i;
	struct termios options;
...
/* open the serial port */
#if 0
                     SYSCHECK(serial->fd = open(serial->devicename, O_RDWR));
#else

    SYSCHECK(serial->fd = open(serial->devicename, O_RDWR | O_NOCTTY | O_NDELAY));
    //SYSCHECK(serial->fd = open(serial->devicename, O_RDWR));
    LOGMUX(LOG_WARNING,"!!!!!!!!!!!!!!1   file:gsm0710.c  setAdvancedOptions()   !!!!!!!!!!!!!!!!!!\n");
	...
	)
{
	...
	if (chat(serial->fd, "AT\r\n", 1) < 0)
	{
		...
		//if (cmux_mode) we do not know now so write both
		       //LOGMUX(LOG_INFO,"call write_frame()     .10 ");
			write_frame(0, NULL, 0, GSM0710_CONTROL_CLD | GSM0710_CR);
		//else
		       //LOGMUX(LOG_INFO,"call write_frame()     .11 ");
...
#if 1
       SYSCHECK(snprintf(gsm_command, sizeof(gsm_command), "AT+CMUX=0\r\n"));
       //SYSCHECK(snprintf(gsm_command, sizeof(gsm_command), "AT+CMUX=0,0,5,1500\r\n"));
       //SYSCHECK(snprintf(gsm_command, sizeof(gsm_command), "AT+CMUX=0,0,5,1500\r\n"));
#else
...
//the mux mode
		if (channellist[i].fd >= 0)
		{
			if (channellist[i].opened)
			{
				...
				if (cmux_mode){
					//LOGMUX(LOG_INFO,"call write_frame()     .12 ");
					write_frame(i, NULL, 0, GSM0710_CONTROL_CLD | GSM0710_CR);
					}
				else{
					//LOGMUX(LOG_INFO,"call write_frame()     .13 ");
...
static int watchdog(Serial * serial)
{
	...
	case MUX_STATE_MUXING:
		...
		if (pts_reopen==1){
			LOGMUX(LOG_INFO, "Watchdog   MUX_STATE_MUXING  pts_reopen==1 !!!!!!!\r\n");
			for(i=1;i<=vir_ports;i++){
		...
		if (use_ping)
		{
		       LOGMUX(LOG_INFO, "Watchdog   MUX_STATE_MUXING  use_ping==!!!!!!!\r\n");
		...
		if (use_timeout)
		{
		       LOGMUX(LOG_INFO, "Watchdog   MUX_STATE_MUXING  use_timeout==!!!!!!!\r\n");
			time_t current_time;
...
void* poll_thread(void *vargp) {
  ...
  while (1)
  {
    ...
    //FD_SET(poll_thread_arg->fd,&fdw);
    //LOGMUX(LOG_EMERG, "poll_thread  test\r\n");
    if ((ret = select((1+poll_thread_arg->fd),&fdr,/*&fdw*/NULL,NULL,&tv))>0) {
      if (FD_ISSET(poll_thread_arg->fd,&fdr)) {
        if((*(poll_thread_arg->read_function_ptr))(poll_thread_arg->read_function_arg)!=0){ /*Call reading function*/
          LOGMUX(LOG_EMERG, "Device read function returned error");
          goto terminate;
        }
      }
      else{
	  //LOGMUX(LOG_EMERG, "poll_thread  (select > 0)  FD_ISSET  not can read file\r\n");	
      }
    }
    //add  for select timeout
    else if(ret == 0){
	  //LOGMUX(LOG_EMERG, "%d.poll_thread  (select == 0)  timeout go on!!!!\r\n",i);
	  i++;
    }
    //end of add
...
int main(int argc,char *argv[])
{
...
#if 1 //PTS_NAME_LINK
	while ((opt = getopt(argc, argv, "dov:s:S:t:p:f:n:h?m:b:P:")) > 0)
#else
...
#endif
	{
		...
		case 'v':
			...
			fprintf(stderr,"gsm0710muxd_main == %s\n",syslog_level);
			LOGMUX(LOG_INFO,"gsm0710muxd_main == %s\n",syslog_level);
			if ((syslog_level>LOG_DEBUG) || (syslog_level < 0)){
			fprintf(stderr,"gsm0710muxd_main_exit\n");
			LOGMUX(LOG_INFO,"gsm0710muxd_main_exit\n");
                     usage(argv[0]);
			  exit(0);
			#ifdef MUX_ANDROID
		...
		case 'o':
			...
			if ((muxlogfile=fopen("/tmp/gsm0710muxd.log", "w+")) == NULL){
				 ...
				 LOGMUX(LOG_INFO,"Error: %s.\n", strerror(errno));
				 usage(argv[0]);
				 exit(0);
				 }
			else
				fprintf(stderr, "gsm0710muxd log is output to /tmp/gsm0710muxd.log\n");
			       LOGMUX(LOG_INFO,"gsm0710muxd log is output to /tmp/gsm0710muxd.log\n");
			break;			
		case 'd':
			no_daemon = !no_daemon;
			LOGMUX(LOG_INFO,"gsm0710muxd_main 'd'== %d\n",no_daemon);
			break;
		case 's':   //物理传输设备文件名 如:"/dev/misc/hif"
			serial.devicename = optarg;
			LOGMUX(LOG_INFO,"gsm0710muxd_main 's'== %s\n",serial.devicename);
			break;
		//#if PTS_NAME_LINK
		case 'S':   //逻辑通道自定义的符号链接名称前缀 @dxj
			ptsSymlinkPre = optarg;
			LOGMUX(LOG_INFO,"gsm0710muxd_main 'S'== %s\n",ptsSymlinkPre);
			break;
		//#endif
		case 't':   
			use_timeout = atoi(optarg);
			LOGMUX(LOG_INFO,"gsm0710muxd_main 't'== %d\n",use_timeout);
			break;
		case 'p':
			use_ping = atoi(optarg);
			LOGMUX(LOG_INFO,"gsm0710muxd_main 'p'== %d\n",use_ping);
			break;
		case 'P':
		...
		case 'f':   //framesize 的N1数值
			cmux_N1 = atoi(optarg);
			LOGMUX(LOG_INFO,"gsm0710muxd_main 'f'== %d\n",cmux_N1);
			break;
		case 'n':
			vir_ports = atoi(optarg);
			LOGMUX(LOG_INFO,"gsm0710muxd_main 'n'== %d\n",vir_ports);
			...
			break;
		case 'm':
			LOGMUX(LOG_INFO,"gsm0710muxd_main 'm'== %s\n",optarg);
		...
		case 'b':
			...
			LOGMUX(LOG_INFO,"gsm0710muxd_main 'b'== %d\n",cmux_port_speed);
			break;
		default:
		case '?':
		case 'h':
